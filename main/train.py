...
def train_model(dataset_param, model_param, general_param, loss_func=F.mse_loss):
    ...
    if resume_flag and len([fn for fn in os.listdir(log_dir_name) if 'model' in fn and '.pth' in fn]) > 0:
        ...
        if len(model_filename):
            ...
            test_loss_hist, test_gen_loss_hist = checkpoint['test_loss_hist'], checkpoint['test_gen_loss_hist']
    ...
    for epoch in range(epoch+1, epoch_num+1):
        ...
        for step,data in tqdm(enumerate(data_loader)):
            ...
            y = data.y
            loss = loss_func(pred.reshape(y.size()), y)
            loss.backward()
            optimizer.step()
        ...
        if epoch % save_freq == 0:
            torch.save({
                ...
                'test_gen_loss_hist':test_gen_loss_hist,
                'layer_num_hist':layer_num_hist,
            }, os.path.join(log_dir_name, 'model_%d.pth'%epoch))
